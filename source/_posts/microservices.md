...
简而言之，**微服务架构风格**[^1]是一个开发单应用作为小型服务套件开发模式，每个应用运行在自己的进程中并且他们之间通过轻量级的机制进行通信，通常的如 HTTP 资源 API。这些服务围绕业务能力并且这些都是可以独立的自动化部署。这些服务我们基本很少进行中央化的集中管理。这些服务可以使用不同的编程语言来编写同样也可以使用不同的数据存储技术
